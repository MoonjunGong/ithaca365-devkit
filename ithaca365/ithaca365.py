# Ithaca-365 dev-kit.
# Code written based on nuScenes dev-kit
...
if not PYTHON_VERSION == 3:
    raise ValueError("Ithaca365 dev-kit only supports Python version 3.")

class Ithaca365(object):
    """
    ...
    @property
    def sample_with_accurate_localization(self):
        return [sample for sample in self.sample if
                self.get("sample_data", sample['data']['LIDAR_TOP'])[
                    'bestpos']['field.pos_type.type'] >= 56.
                ]
    ...
    def query_by_location_and_channel(self, location_token, channel,
                                      sorted_by='time', increasing_order=True):
        """
        Returns sample_tokens with specific location token and sensor channel
        """
        token_list = [v[channel] for k, v in self.get('location', location_token)['sample_data_dict'].items()]
        if sorted_by == "time":
            time_list = [self.get("sample_data", tkn)['timestamp'] for tkn in token_list]
            idx_list = np.argsort(time_list)
        elif sorted_by == "hgt_stdev":
            hgt_stdev = [self.get("sample_data", tkn)['bestpos']['field.hgt_stdev']
                         for tkn in token_list]
            idx_list = np.argsort(hgt_stdev)
        elif sorted_by == "pos_type":
            hgt_stdev = [self.get("sample_data", tkn)['bestpos']['field.pos_type.type']
                         for tkn in token_list]
            idx_list = np.argsort(hgt_stdev)
        else:
        ...
        if not increasing_order:
            idx_list = idx_list[::-1]
        token_list = [token_list[i] for i in idx_list]
        return token_list
    ...
    def get_lidar_within_range(self, sample_data_token, ranges=(0, 70), every_x_meter=5.):
        """
        Returns lists of LiDAR that are within requested range (back, front) in
        the route. The LiDARs will be in the same coordinate system as the input
        sample_data.
        """
        ...
        _current_ego2world = ego2world
        _current_sd = sample_data
        _target_dist = every_x_meter
        bw_lidars = []
        bw_sd_tokens = []
        # backward
        while np.linalg.norm(_current_ego2world[:3, 3] - ego2world[:3, 3]) < np.abs(ranges[0]) \
            and _current_sd['prev'] != '':
            _current_sd = self.get('sample_data', _current_sd['prev'])
            _current_ego_pose = self.get('ego_pose', _current_sd['ego_pose_token'])
            _current_ego2world = transform_matrix(
                _current_ego_pose['translation'],
                Quaternion(_current_ego_pose['rotation']),)
            if np.linalg.norm(_current_ego2world[:3, 3] - ego2world[:3, 3]) < _target_dist:
                continue
            lidar = load_velo_scan(osp.join(self.dataroot, _current_sd['filename']))
            lidar[:, :3] = transform_points(lidar[:, :3], ego2lidar @ world2ego @ _current_ego2world @ lidar2ego)
            bw_lidars.append(lidar)
            bw_sd_tokens.append(_current_sd['token'])
            _target_dist += every_x_meter
        bw_lidars = bw_lidars[::-1]
        bw_sd_tokens = bw_sd_tokens[::-1]
        # forward
        _current_ego2world = ego2world
        _current_sd = sample_data
        _target_dist = every_x_meter
        fw_lidars = []
        fw_sd_tokens = []
        while np.linalg.norm(_current_ego2world[:3, 3] - ego2world[:3, 3]) < np.abs(ranges[1]) \
            and _current_sd['next'] != '':
            _current_sd = self.get('sample_data', _current_sd['next'])
            _current_ego_pose = self.get('ego_pose', _current_sd['ego_pose_token'])
            _current_ego2world = transform_matrix(
                _current_ego_pose['translation'],
                Quaternion(_current_ego_pose['rotation']),)
            if np.linalg.norm(_current_ego2world[:3, 3] - ego2world[:3, 3]) < _target_dist:
                continue
            lidar = load_velo_scan(osp.join(self.dataroot, _current_sd['filename']))
            lidar = self.remove_ego_points(lidar)
            lidar[:, :3] = transform_points(lidar[:, :3], ego2lidar @ world2ego @ _current_ego2world @ lidar2ego)
            fw_lidars.append(lidar)
            fw_sd_tokens.append(_current_sd['token'])
            _target_dist += every_x_meter
        return bw_lidars + fw_lidars, bw_sd_tokens + fw_sd_tokens
    
    def is_ignored_history(self, sample_data_token):
        return self.get(
            'scene',
            self.get(
                'sample', self.get(
                    'sample_data', sample_data_token)['sample_token']
                )['scene_token']
            )['name'] in self.ignored_history

    def get_point_persistency_score(
            self, sample_data_token, num_histories=5, ranges=(0, 70), every_x_meter=5., max_neighbor_dist=0.3, num_workers=-1
            ):
        # load current lidar data
        sample_lidar = load_velo_scan(self.get_sample_data_path(sample_data_token))  # lidar frame
        ...
        past_traversals = self.get_other_traversals(sample_data_token, num_history=num_histories, 
    ...
    def get_other_traversals(
            self, sample_data_token, num_history=5, ranges=(0, 70), every_x_meter=5.,
            sorted_by='hgt_stdev', increasing_order=True, time_valid=False,
            accurate_history_only=False
            ):
        sample_data = self.get('sample_data', sample_data_token)
        assert sample_data['channel'] == 'LIDAR_TOP'
        ...
        token_list = self.query_by_location_and_channel(
            sample_data['location_token'], 'LIDAR_TOP',
            sorted_by=sorted_by, increasing_order=increasing_order)

        # remove self
        token_list.remove(sample_data['token'])
        if time_valid:
            token_list = [tkn for tkn in token_list
                          if self.get("sample_data", tkn)['timestamp'] < sample_data['timestamp']]
        if accurate_history_only:
            token_list = [tkn for tkn in token_list
                          if self.get("sample_data", tkn)['bestpos']['field.pos_type.type'] >= 56.]
        ...
        for i in range(min(num_history, len(token_list))):
            ...
            _current_lidar = load_velo_scan(self.get_sample_data_path(sample_data_token))
            _current_lidar = self.remove_ego_points(_current_lidar)
            dense_ptc = self.get_lidar_within_range(
                sample_data_token, ranges=ranges, every_x_meter=every_x_meter)[0]
            _current_sd = self.get('sample_data', sample_data_token)
            _current_ego_pose = self.get('ego_pose', _current_sd['ego_pose_token'])
            _current_ego2world = transform_matrix(
                _current_ego_pose['translation'],
                Quaternion(_current_ego_pose['rotation']),)
    ...
    def get_lidar_within_range_tokens(self, sample_data_token, ranges=(0, 70), every_x_meter=5.):
        """
        Returns lists of tokens that are within requested range (back, front) in
        the route. 
        """
        sample_data = self.get('sample_data', sample_data_token)
        ...
        while np.linalg.norm(_current_ego2world[:3, 3] - ego2world[:3, 3]) < np.abs(ranges[0]) \
            ...
            if np.linalg.norm(_current_ego2world[:3, 3] - ego2world[:3, 3]) < _target_dist:
                continue
            bw_sd_tokens.append(_current_sd['sample_token'])
            _target_dist += every_x_meter
        bw_lidars = bw_lidars[::-1]
        bw_sd_tokens = bw_sd_tokens[::-1]
        # forward
        ...
        while np.linalg.norm(_current_ego2world[:3, 3] - ego2world[:3, 3]) < np.abs(ranges[1]) \
            ...
            if np.linalg.norm(_current_ego2world[:3, 3] - ego2world[:3, 3]) < _target_dist:
                continue
            fw_sd_tokens.append(_current_sd['sample_token'])
            _target_dist += every_x_meter
        return bw_sd_tokens + [sample_token]+ fw_sd_tokens
    def get_other_traversals_tokens(
            self, sample_data_token, num_history=5, ranges=(0, 70), every_x_meter=5.,
            sorted_by='hgt_stdev', increasing_order=True, time_valid=False,
            accurate_history_only=False
            ):
        ...
        for i in range(min(num_history, len(token_list))):
            ...
            traversal_tokens_list = self.get_lidar_within_range_tokens(
                sample_data_token, ranges=ranges, every_x_meter=every_x_meter)
            history_tokens[sample_data_token] = traversal_tokens_list
    ...
    def __make_reverse_index__(self, verbose: bool) -> None:
        ...
        for locataion_record in self.location:
            locataion_record['sample_data_dict'] = {}
        # Add reverse indices from location token to sample_data
        for record in self.sample_data:
            if record['location_token'] != '':
                locataion_record = self.get('location', record['location_token'])
                scene_token = self.get('sample', record['sample_token'])['scene_token']
                channel = record['channel']
                if not scene_token in locataion_record['sample_data_dict']:
                    locataion_record['sample_data_dict'][scene_token] = {}
                locataion_record['sample_data_dict'][scene_token][channel] = record['token']
    ...
    def list_locations(self) -> None:
        print('Location stats, total number of locations:', len(self.location))
        for location in self.location:
            print('{}: token {}'.format(location['description'], location['token']))

    def list_weathers(self) -> None:
        print('Weather stats, total number of weather conditions:', len(self.weather))
        for weather in self.weather:
            print('{}: token {}'.format(weather['description'], weather['token']))
    ...
    def list_anns(self, sample_token: str, verbose=False):
        object_anns, surface_anns = self.explorer.list_anns(sample_token)
        return object_anns, surface_anns
    def render_image(self, sd_token: str,
        ...
        self.explorer.render_image(sd_token, annotation_type,
                        with_category,
                        with_attributes,
                        object_tokens,
                        surface_tokens,
...
class NuScenesExplorer:
    ...
    def __init__(self, nusc: Ithaca365):
        self.nusc = nusc
    # def get(self, table_name: str, token: str) -> dict:
    #     """
    #     Returns a record from table in constant runtime.
    ...
    def list_anns(self, sample_token: str, verbose: bool = True) -> Tuple[List[str], List[str]]:
        ...
        object_anns = [o for o in self.nusc.object_ann if o['sample_data_token'] == key_camera_token]
        surface_anns = [o for o in self.nusc.surface_ann if o['sample_data_token'] == key_camera_token]

        if verbose:
            print('Printing object annotations:')
            for object_ann in object_anns:
                category = self.nusc.get('category', object_ann['category_token'])
                attribute_names = [self.nusc.get('attribute', at)['name'] for at in object_ann['attribute_tokens']]
                print('{} {} {}'.format(object_ann['token'], category['name'], attribute_names))

            print('\nPrinting surface annotations:')
            for surface_ann in surface_anns:
                category = self.nusc.get('category', surface_ann['category_token'])
    ...
    def render_annotation(self,
        ...
        boxes, cam = [], []
        cams = [key for key in sample_record['data'].keys() if 'cam' in key.lower()]
        for cam in cams:
    ...
    def render_image(self,
            ...
            # Get image data.
            # self.check_sweeps(sample_data['filename'])
            im_path = osp.join(self.nusc.dataroot, sample_data['filename'])
            im = Image.open(im_path)
            ...
            if annotation_type is not 'none':
                if annotation_type == 'all' or annotation_type == 'surfaces':
                    # Load stuff / surface regions.
                    surface_anns = [o for o in self.nusc.surface_ann if o['sample_data_token'] == sd_token]
                    ...
                    for ann in surface_anns:
                        # Get color and mask.
                        category_token = ann['category_token']
                        category_name = self.nusc.get('category', category_token)['name']
                        color = self.nusc.colormap[category_name]
                        if ann['mask'] is None:
                            continue
                        mask = mask_decode(ann['mask'])
                ...
                if annotation_type == 'all' or annotation_type == 'objects':
                    # Load object instances.
                    object_anns = [o for o in self.nusc.object_ann if o['sample_data_token'] == sd_token]
                    ...
                    for ann in object_anns:
                        ...
                        attributes = [self.nusc.get('attribute', at) for at in attr_tokens]
